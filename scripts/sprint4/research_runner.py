"""Research runner for Sprint 4 (anomaly-first, low FPR, strict zero-shot)."""
...
def derive_isolated_paths(run_id: str) -> dict[str, str]:
    processed_dir = Path("data") / "sprint4" / run_id / "processed"
    return {
        "data_processed": str(processed_dir).replace("\\", "/"),
        "shard_dir": str(processed_dir / "shards").replace("\\", "/"),
        "models_dir": str(Path("models") / "sprint4" / run_id).replace("\\", "/"),
        "results_dir": str(Path("results") / "sprint4" / run_id).replace("\\", "/"),
    }
...
def lock_root_path() -> Path:
    ...
    if colab_root.exists():
        return colab_root / "sprint4_lock"
    return Path(tempfile.gettempdir()) / "sprint4_lock"
...
def run_single_experiment(
    ...
    for stage in stages:
        if stage == "preprocess":
            total_sec += run_cmd(
                [python_exe, "scripts/sprint4/preprocess.py", "--config", str(run_cfg_path)],
                cwd=root,
                dry_run=dry_run,
            )
        elif stage == "train":
...
def main() -> None:
    ...
    parser.add_argument("--registry", default="research/sprint4/run_registry.yaml")
    parser.add_argument("--base-config", default="config/sprint4/base.yaml")
    ...
    parser.add_argument("--summary-csv", default="results/sprint4/summary.csv")
    parser.add_argument("--report-md", default="docs/sprint4/RESEARCH_REPORT_CSE_F1_SPRINT4.md")
    parser.add_argument("--gate-json", default="results/sprint4/gate_decision.json")
    parser.add_argument("--status-json", default="results/sprint4/runtime/run_status.json")
    parser.add_argument("--eta-json", default="results/sprint4/eta_stage2.json")
